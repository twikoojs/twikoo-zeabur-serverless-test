...
exports.copyFile = exports.getFileAuthority = exports.getUploadMetadata = exports.downloadFile = exports.getTempFileURL = exports.deleteFile = exports.uploadFile = exports.parseXML = void 0;
...
async function uploadFile(cloudbase, { cloudPath, fileContent }, opts) {
    ...
    if (body && body.Error) {
        ...
        if (code === 'SignatureDoesNotMatch') {
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.SYS_ERR), { message: `[${code}]: ${message}`, requestId: `${requestId}|${cosRequestId}|${cosTraceId}` }));
        }
        return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.STORAGE_REQUEST_FAIL), { message: `[${code}]: ${message}`, requestId: `${requestId}|${cosRequestId}|${cosTraceId}` }));
    }
    return {
        fileID: fileId
    };
...
async function deleteFile(cloudbase, { fileList }, opts) {
    if (!fileList || !Array.isArray(fileList)) {
        return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'fileList必须是非空的数组' }));
    }
    for (let file of fileList) {
        if (!file || typeof file !== 'string') {
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'fileList的元素必须是非空的字符串' }));
        }
    }
    let params = {
    ...
    for (let file of fileList) {
        if (typeof file === 'object') {
            if (!file.hasOwnProperty('fileID') || !file.hasOwnProperty('maxAge')) {
                return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'fileList的元素如果是对象，必须是包含fileID和maxAge的对象' }));
            }
            file_list.push({
                fileid: file.fileID,
                max_age: file.maxAge
            ...
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'fileList的元素如果不是对象，则必须是字符串' }));
        }
    }
    let params = {
...
async function downloadFile(cloudbase, params, opts) {
    ...
    if (res.code !== 'SUCCESS') {
        return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign({}, res));
    }
    tmpUrl = res.tempFileURL;
    tmpUrl = encodeURI(tmpUrl);
    let req = (0, request_1.default)({
        url: tmpUrl,
        encoding: null,
        proxy: cloudbase.config.proxy
    });
...
async function getUploadMetadata(cloudbase, { cloudPath }, opts) {
    ...
    const res = await (0, httpRequest_1.default)({
        config: cloudbase.config,
        params,
        method: 'post',
        opts,
...
async function copyFile(cloudbase, fileList, opts) {
    ...
    for (let file of fileList) {
        ...
        if (!srcPath || !dstPath || typeof srcPath !== 'string' || typeof dstPath !== 'string') {
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'srcPath和dstPath必须是非空的字符串' }));
        }
        if (srcPath === dstPath) {
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'srcPath和dstPath不能相同' }));
        }
        if (path_1.default.basename(srcPath) !== path_1.default.basename(dstPath)) {
            return (0, utils_1.processReturn)(cloudbase.config.throwOnCode, Object.assign(Object.assign({}, code_1.ERROR.INVALID_PARAM), { message: 'srcPath和dstPath的文件名必须相同' }));
        }
        list.push({
            src_path: srcPath,
            dst_path: dstPath,
